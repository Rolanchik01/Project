...
`crates/live` умеет `logsSubscribe` и для Pump, и для PumpSwap (два отдельных демо-бинарника, `bin/pump_listener.rs` и `bin/pumpswap_listener.rs` — один и тот же venue-agnostic `listener::run`, просто с разным `program_id`; каждый свой отдельный процесс, общего процесса на оба venue пока нет), печатает декодированные события. Теперь умеет и `accountSubscribe` (`account_watcher`/`account_notification`) — с динамическим набором отслеживаемых аккаунтов (`Watch`/`Unwatch` через отдельный командный канал, переподписка на весь текущий набор при переподключении), проверено живьём на реальном mainnet (`bin/account_watcher_demo.rs`, живая подписка на реальный PumpSwap-пул плюс динамическая замена одного аккаунта на другой во время работы). Ничего из этого пока не подключено к `crates/ingest`/risk-engine/`recorder` (эта склейка ещё впереди) — и, что важнее, нет слоя, который решает, *какие именно* аккаунты смотреть, когда новый bonding curve/pool/mint появляется из живых `TokenCreated`/`Trade` событий (`account_watcher` только следит за тем, что ему явно сказали). Нет резервного провайдера (Geyser/Helius) на случай проблем с публичным RPC.
